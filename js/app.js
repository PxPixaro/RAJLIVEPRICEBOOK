...
// V82.2 PDF alignment + image safety patch ---------------------------------
// Keep the PDF header on fixed left/center/right anchors and read the real JPEG
// dimensions before embedding. This avoids Acrobat "Insufficient data for an image"
// errors caused by declaring dimensions that do not match the encoded JPEG.
...
function pdfImageObject(bytes){
  const d=pdfJpegDimensions(bytes);
  return {bin:bytes,head:`<< /Type /XObject /Subtype /Image /Width ${d.width} /Height ${d.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${bytes.length} >>`};
}
function pdfTextWidth(text,fontSize,bold=false){
  // Helvetica-compatible width estimate; stable enough for visual centering.
  const s=pdfAscii(text),factor=bold?.555:.515;
  return s.length*fontSize*factor;
}
function pdfCenteredText(text,fontSize,y,font='F2',color='0 0 0',centerX=421){
  const x=Math.max(120,centerX-pdfTextWidth(text,fontSize,font==='F2')/2);
  return `BT /${font} ${fontSize} Tf ${color} rg ${x.toFixed(2)} ${y} Td (${pdfAscii(text)}) Tj ET`;
}
